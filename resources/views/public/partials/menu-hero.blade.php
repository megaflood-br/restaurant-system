...
<header class="relative bg-gray-50">
    {{-- Capa --}}
    <div class="relative h-44 sm:h-52 w-full overflow-hidden bg-gradient-to-br from-orange-400 via-orange-500 to-red-500">
        @if ($menu['cover_url'])
    ...
    <div class="relative -mt-10 px-4 pb-4">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 px-4 pt-12 pb-4 text-center">
            ...
            <div class="absolute left-1/2 -translate-x-1/2 -top-8">
                ...
                @else
                    <div class="h-20 w-20 rounded-full border-4 border-white shadow-md bg-orange-500 text-white flex items-center justify-center text-2xl font-bold">
            ...
            @if ($menu['city'] || $menu['state'])
                <p class="mt-1 text-sm text-gray-500 flex items-center justify-center gap-1 flex-wrap">
                    ...
                    @if ($menu['more_info'])
                        ...
                        <button type="button" @click="infoOpen = true" class="text-orange-600 font-medium">Mais informações</button>
                    @endif
                </p>
            @endif
            ...
            @isset($comandaNumber)
                @if($comandaNumber)
                    <span class="inline-flex mt-2 items-center rounded-full bg-orange-100 px-3 py-1 text-xs font-semibold text-orange-800">
        ...
        @if ($menu['address_line'])
            <a href="{{ route('public.checkout') }}"
                class="mt-3 flex items-center gap-3 rounded-2xl border border-gray-200 bg-white px-4 py-3 shadow-sm hover:border-orange-200 transition">
            ...
            <div class="mt-3 rounded-2xl border border-gray-200 bg-white px-4 py-4 shadow-sm">
                @if ($menu['loyalty_title'])
                    <p class="font-semibold text-gray-900 flex items-center gap-2">
                        <svg class="w-5 h-5 text-orange-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v13m0-13V6a4 4 0 00-4-4H5a2 2 0 00-2 2v6a2 2 0 002 2h2m8-8V6a4 4 0 014-4h1a2 2 0 012 2v6a2 2 0 01-2 2h-2"/></svg>
                        {{ $menu['loyalty_title'] }}
                    </p>
                @endif
    ...
    @if ($menu['more_info'])
        <div x-show="infoOpen" x-cloak class="fixed inset-0 z-[70] flex items-end sm:items-center justify-center p-4 bg-black/50"
            ...
            <div @click.outside="infoOpen = false" class="bg-white rounded-2xl w-full max-w-sm p-5 shadow-xl">
                ...
                <button type="button" @click="infoOpen = false" class="mt-4 w-full rounded-xl bg-orange-500 text-white font-semibold py-3">Fechar</button>
            </div>
        </div>
    @endif
</header>
