@props(['selected' => null, 'compact' => false])
...
<fieldset {{ $attributes->merge(['class' => 'space-y-2']) }}>
    <legend class="block text-sm font-medium text-gray-700 mb-2">Forma de pagamento *</legend>
    <div @class([
        'grid gap-2',
        ...
        @foreach ($methods as $value => $label)
            <label class="cursor-pointer">
                <input type="radio" name="payment_method" value="{{ $value }}" class="peer sr-only" required
                    @checked(old('payment_method', $selected) === $value)>
                <span @class([
