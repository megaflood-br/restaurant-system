...
class OpenAiWhatsAppAgentService
{
    ...
    private function buildMessages(string $phone, ?string $pushName): array
    {
        ...
        $system = implode("\n", [
            ...
            'Se session.cart já contiver os itens do texto do cliente nesta mensagem, NÃO chame add_to_cart de novo (evita duplicar quantidade). Confirme o que já está no carrinho e pergunte se quer mais alguma coisa ou digite pronto.',
            'Se add_to_cart retornar already_in_cart=true, os itens já estavam no carrinho — confirme sem dizer que adicionou de novo.',
            'Se session.cart já contiver itens, use SOMENTE esses nomes — não substitua por outro prato do cardápio resumido.',
            'Se o estado da sessão for "ordering" e o cliente responder só com acompanhamento (fritas/legumes), NÃO chame add_to_cart: use finalize_items e depois set_side.',
